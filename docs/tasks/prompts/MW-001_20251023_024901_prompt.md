# Task Implementation Request: MW-001 - Authentication Middleware
...
- **Dependencies**: GIN-001
  - Must complete before starting: GIN-001
  - Related tasks: - GIN-001: Setup Gin router

### Task Description
Implement a Gin middleware that validates incoming JWTs. It must verify the token's signature, expiry, and other standard claims. Upon successful validation, the middleware will extract user identity and relevant claims (e.g., `UserID`, `Roles`) and attach them to the `gin.Context` for subsequent handlers. Invalid or missing tokens should result in appropriate HTTP 401 Unauthorized responses.

### Acceptance Criteria
1. JWT auth validates token, extracts `UserContext` from `gin.Context`.
2. Authorization checks user roles/permissions.
3. CORS middleware configured with env-specific settings.
4. Rate limiting prevents abuse per endpoint.
5. Recovery middleware catches panics, returns 500.
...
****Web/API Layer** - Handles HTTP request processing, authentication, authorization, and common cross-cutting concerns.** (e.g., Repository Layer, Service Layer, etc.)
...
```
- internal/middleware/auth.go - JWT validation, user context, authorization.
- internal/middleware/gin.go - Gin middleware integration.
Example:
...
```go
```go
// Authenticator defines core methods for JWT validation and user context processing.
type Authenticator interface {
	ValidateToken(token string) (any, error)
	ExtractUserContext(claims any) (UserContext, error)
...
- github.com/spf13/viper - Configuration management for JWT settings
...
CORS_ALLOWED_ORIGINS=http://localhost:3000,https://go.flow.org
...
1. ✅ Task dependencies are complete (check GIN-001)
2. ✅ Related documentation sections:
   - 1. `github.com/golang-jwt/jwt/v5`
2. Gin Middleware Development Guide
...
Create test file: `internal/middleware/auth_test.go`
...
**File**: `test/integration/Authentication Middleware_test.go`
...
```go
// Package middleware provides {{DESCRIPTION}}.
//
...
```markdown
| MW-001 | Authentication Middleware | {{DESCRIPTION}} | P0 | ✅ | GIN-001 | 100% |
```
...
```markdown
| MW-001 | Authentication Middleware | {{YOUR_NAME}} | 2025-10-23 |
```
...
**File**: `docs/summaries/MW-001_Authentication Middleware_SUMMARY.md`
